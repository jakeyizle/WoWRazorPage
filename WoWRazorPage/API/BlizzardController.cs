...
namespace WoWRazorPage.API
{
    ...
    public class BlizzardController : ControllerBase
    {
        ...
        public async Task<List<Zone>> GetFullAsync(double mainStatWeight, double critWeight, double hasteWeight, double masteryWeight, double versatilityWeight, string mainStatName)
        {
            ...
            using (StreamReader r = new StreamReader("C:/Development/WebAPI/WebAPI/Resources/characterItems.json"))
            {
            ...
            using (StreamReader r = new StreamReader("C:/Development/WebAPI/WebAPI/Resources/bosses.json"))
            {
                string json = r.ReadToEnd();
            ...
            using (StreamReader r = new StreamReader("C:/Development/WebAPI/WebAPI/Resources/zones.json"))
            {
                string json = r.ReadToEnd();
            ...
            foreach (Zone zone in zones)
            {
                ...
                {
                    boss.items = upgradeItems.Where(y => y.sourceId == boss.id).ToList();
                }
                zone.bosses = zoneBosses.Where(x => x.items.Count() > 0).ToList();
            }
            return zones.Where(x=>x.bosses.Count() > 0).ToList();
        }
        ...
        //public async Task<Object> GetAsync(string characterName, string realm)
        //{
